...
func init() {
	concept.Register(concept.Concept{
		...
		Blurb: "Drop a ball on the side of a bowl-shaped valley and let physics take over: " +
			...
			"Gradient descent is that idea as an algorithm — at each step, move against the " +
			"slope by an amount set by the 'learning rate'. Starting at x=4.5 on f(x)=x²: at " +
			"learning rate 0.3 the steps go 4.5 -> 1.8 -> 0.72 -> 0.29, converging fast. At " +
			"learning rate 1.1 — past this bowl's critical value of 1.0 — the same start goes " +
			"4.5 -> -5.4 -> 6.48 -> -7.78: flipping sign and growing every step, flying further " +
			"from the minimum with every bounce instead of settling. Drag the learning rate and " +
			"watch the same starting point converge, crawl, or blow up.",
