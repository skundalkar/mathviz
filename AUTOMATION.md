...
## Storage, rendering & deployment — the short version

- **Storage:** just this git repo (`skundalkar/mathviz`, public, ~2MB). No
  database, no image store, no CDN.
- **Rendering:** no picture is ever stored. Each concept is a pure Go
  `render(params) → SVG string` function; it's compiled to WebAssembly and
  runs **in the visitor's browser**, recomputing the SVG on every slider
  move. What's committed is the *generator code*, not images.
- **Deployment:** `.github/workflows/pages.yml` builds the WASM and publishes
  `web/` to **GitHub Pages** on every push to `main`. Confirmed live at
  **https://skundalkar.github.io/mathviz/** — Pages is enabled and the last
  5 deploys have succeeded.
- **Cost / capacity:** free — public repos get unlimited Actions minutes and
  free Pages hosting (soft caps: ~1GB site, ~100GB/month bandwidth). Current
  usage is a rounding error against those limits.
- **Security:** public repo, standard GitHub access control, no secrets
  stored — appropriate since the content is just math explanations and SVG
  code.
- **Recovery:** git is fully distributed — any clone (this loop's daily
  clone, or your own) holds complete history and can repopulate a fresh
  remote. Pages has no separate state to lose: re-running the workflow
  rebuilds and republishes from source. The loop itself is stateless
  between runs; progress lives in `BACKLOG.md`, not in any session.
